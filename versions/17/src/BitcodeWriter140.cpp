...
private:
  ...
  void writeDILabel(const DILabel *N,
                    SmallVectorImpl<uint64_t> &Record, unsigned Abbrev);
  void writeDIAssignID(const DIAssignID *N,
                       SmallVectorImpl<uint64_t> &Record, unsigned Abbrev);
...
// LLVM 16 unified the legacy readnone/readonly/writeonly/argmemonly/
// inaccessiblememonly/inaccessiblemem_or_argmemonly attributes into a single
// `memory(...)` (Attribute::Memory) int-attribute. LLVM 14's bitcode reader
// does not understand ATTR_KIND_MEMORY, so decompose the memory effects back
// into the legacy enum-attribute records it does understand. A single
// MemoryEffects value can imply several legacy kinds, so emit one record each.
static void writeMemoryEffectsAsLegacyAttrs(SmallVectorImpl<uint64_t> &Record,
                                            MemoryEffects ME) {
  ...
  if (ME.onlyReadsMemory()) {
    ...
    Record.push_back(0);
    Record.push_back(bitc::ATTR_KIND_WRITEONLY);
  }
  if (ME.onlyAccessesInaccessibleOrArgMem()) {
    Record.push_back(0);
    Record.push_back(bitc::ATTR_KIND_INACCESSIBLEMEM_OR_ARGMEMONLY);
  } else if (ME.onlyAccessesArgPointees()) {
    Record.push_back(0);
    Record.push_back(bitc::ATTR_KIND_ARGMEMONLY);
  } else if (ME.onlyAccessesInaccessibleMem()) {
    Record.push_back(0);
    Record.push_back(bitc::ATTR_KIND_INACCESSIBLEMEM_ONLY);
  }
}
  ...
  for (ValueEnumerator140::IndexAndAttrSet Pair : AttrGrps) {
    ...
    for (Attribute Attr : AS) {
      if (Attr.hasAttribute(Attribute::Memory)) {
        writeMemoryEffectsAsLegacyAttrs(Record, Attr.getMemoryEffects());
...
void ModuleBitcodeWriter140::writeTypeTable() {
  ...
  for (Type *T : TypeList) {
    ...
    case Type::PointerTyID: {
      ...
      // pointer is emitted with an opaque (empty struct) element type.
...
void ModuleBitcodeWriter140::writeDIAssignID(
    const DIAssignID * /*N*/, SmallVectorImpl<uint64_t> & /*Record*/,
    unsigned /*Abbrev*/) {
  // DIAssignID (assignment tracking) was introduced after LLVM 14 and has no
  // bitcode record the genuine LLVM 14 reader understands. It only appears via
  // llvm.dbg.assign attachments, which the AIR downgrade path does not produce.
  llvm_unreachable("DIAssignID is not supported by the LLVM 14 bitcode writer");
}
...
void IndexBitcodeWriter140::writeModStrings() {
  ...
  SmallVector<unsigned, 64> Vals;
  forEachModule([&](const StringMapEntry<std::pair<uint64_t, ModuleHash>>
                        &MPSE) {
    StringRef Key = MPSE.getKey();
    const auto &Hash = MPSE.getValue().second;
...
BitcodeWriter140::BitcodeWriter140(SmallVectorImpl<char> &Buffer, raw_fd_stream *FS)
    : Buffer(Buffer), Stream(new BitstreamWriter(Buffer, FS, /*FlushThreshold(MB)=*/512)) {
