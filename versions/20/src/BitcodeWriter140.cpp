  ...
  for (ValueEnumerator140::IndexAndAttrSet Pair : AttrGrps) {
    ...
    for (Attribute Attr : AS) {
      if (Attr.hasAttribute(Attribute::Memory)) {
        // LLVM 14 has no memory(...) attribute; decompose the MemoryEffects
        // into the legacy standalone enum attributes the LLVM 14 reader knows.
        ...
        // Attribute kinds introduced after LLVM 14 (e.g. the range/initializes
        // ConstantRange(-list) attributes from LLVM 18/19, or captures from
        // LLVM 21) have no LLVM 14 encoding -- and Metal's AIR loader rejects
        // them anyway. They are optimization hints, so drop them rather than
        // asserting/emitting an unreadable record.
      }
    }
...
void ModuleBitcodeWriter140::writeTypeTable() {
  ...
  for (Type *T : TypeList) {
    ...
    case Type::TargetExtTyID:
      llvm_unreachable("Target extension types are not supported with LLVM 14");
      break;
    }
...
void ModuleBitcodeWriter140::writeDIAssignID(const DIAssignID *N,
  ...
  llvm_unreachable("DIAssignID is not supported with LLVM 14");
}
