...
private:
  ...
  void writeDILocation(const DILocation *N, SmallVectorImpl<uint64_t> &Record,
                       unsigned &Abbrev);
  // DIAssignID did not exist in LLVM 14; a genuine LLVM 14 module can never
  // contain one. It only appears in this switch because we share the live
  // (LLVM 16) Metadata.def. Reject it rather than emit a record LLVM 14 cannot
  // read.
  ...
  for (ValueEnumerator140::IndexAndAttrSet Pair : AttrGrps) {
    ...
    for (Attribute Attr : AS) {
      if (Attr.hasAttribute(Attribute::Memory)) {
        // LLVM 14 has no unified `memory(...)` attribute; it is removed in
        // LLVM 16. Decompose the MemoryEffects into the equivalent legacy
        // enum attributes that genuine LLVM 14 bitcode understands.
        MemoryEffects ME = Attr.getMemoryEffects();
        auto EmitEnum = [&](uint64_t Enc) {
          Record.push_back(0);
...
void ModuleBitcodeWriter140::writeDIAssignID(const DIAssignID *N,
                                             ...
                                             unsigned Abbrev) {
  report_fatal_error(
      "DIAssignID metadata is not representable in LLVM 14 bitcode");
}
