...
public:
  ...
  ModuleBitcodeWriterBase140(const Module &M, StringTableBuilder &StrtabBuilder,
    ...
    // Enumerate typed pointers (LLVM 22 only has opaque pointers).
...
private:
  ...
  void writeDIGenericSubrange(const DIGenericSubrange *N,
                              SmallVectorImpl<uint64_t> &Record,
                              unsigned Abbrev);
  // The following node kinds postdate LLVM 14 and have no representation in
  // genuine LLVM 14 bitcode. They are referenced by the (LLVM 22) Metadata.def
  // dispatch table but must never occur in a module being downgraded.
...
void ModuleBitcodeWriter140::writeAttributeGroupTable() {
  ...
  for (ValueEnumerator140::IndexAndAttrSet Pair : AttrGrps) {
    ...
    for (Attribute Attr : AS) {
      ...
      // LLVM 14 only has nocapture, equivalent to captures(none); drop any
      // weaker capture information.
      if ((Attr.isEnumAttribute() || Attr.isIntAttribute()) &&
          Attr.getKindAsEnum() == Attribute::Captures) {
      ...
      // LLVM 16 replaced readnone/readonly/writeonly/argmemonly/
      // inaccessiblememonly/inaccessiblemem_or_argmemonly with a single
      // memory(...) effect attribute. LLVM 14 only knows the legacy enum
      // attributes, so decode the effect set back into them.
      if ((Attr.isEnumAttribute() || Attr.isIntAttribute()) &&
          Attr.getKindAsEnum() == Attribute::Memory) {
        MemoryEffects ME = Attr.getMemoryEffects();
        auto emit = [&](uint64_t Kind) {
          Record.push_back(0);
          Record.push_back(Kind);
        };
        if (ME.doesNotAccessMemory())
          emit(bitc::ATTR_KIND_READ_NONE);
        else if (ME.onlyAccessesArgPointees())
          emit(bitc::ATTR_KIND_ARGMEMONLY);
        else if (ME.onlyAccessesInaccessibleMem())
          emit(bitc::ATTR_KIND_INACCESSIBLEMEM_ONLY);
        else if (ME.onlyAccessesInaccessibleOrArgMem())
          emit(bitc::ATTR_KIND_INACCESSIBLEMEM_OR_ARGMEMONLY);
        if (ME.onlyReadsMemory() && !ME.doesNotAccessMemory())
          emit(bitc::ATTR_KIND_READ_ONLY);
        else if (ME.onlyWritesMemory() && !ME.doesNotAccessMemory())
          emit(bitc::ATTR_KIND_WRITEONLY);
        continue;
      }
      if (Attr.isEnumAttribute()) {
        Record.push_back(0);
        Record.push_back(getAttrKindEncoding140(Attr.getKindAsEnum()));
        ...
        // Attribute kinds introduced after LLVM 14 (range/initializes/captures)
...
void ModuleBitcodeWriter140::writeTypeTable() {
  ...
  for (Type *T : TypeList) {
    ...
    case Type::PointerTyID: {
      ...
      // pointer is emitted with an opaque (empty struct) element type. In
      // LLVM 22 all PointerTypes are opaque.
...
// These node kinds were introduced after LLVM 14 and cannot be represented in
// genuine LLVM 14 bitcode. They are only reachable from the (LLVM 22)
// Metadata.def dispatch table; a module targeting LLVM 14 must not contain them.
void ModuleBitcodeWriter140::writeDIAssignID(const DIAssignID *,
                                             SmallVectorImpl<uint64_t> &,
                                             unsigned) {
  report_fatal_error("DIAssignID is not supported by the LLVM 14 bitcode writer");
}

void ModuleBitcodeWriter140::writeDISubrangeType(const DISubrangeType *,
                                                 SmallVectorImpl<uint64_t> &,
                                                 unsigned) {
  report_fatal_error(
      "DISubrangeType is not supported by the LLVM 14 bitcode writer");
}

void ModuleBitcodeWriter140::writeDIFixedPointType(const DIFixedPointType *,
                                                   SmallVectorImpl<uint64_t> &,
                                                   unsigned) {
  report_fatal_error(
      "DIFixedPointType is not supported by the LLVM 14 bitcode writer");
}
...
void ModuleBitcodeWriterBase140::writePerModuleGlobalValueSummary() {
  ...
  for (auto &S : Index->typeIdCompatibleVtableMap()) {
    writeTypeIdCompatibleVtableSummaryRecord(NameVals, StrtabBuilder, std::string(S.first),
                                             S.second, VE);
...
BitcodeWriter140::BitcodeWriter140(SmallVectorImpl<char> &Buffer)
